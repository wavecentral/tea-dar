...
`dar.html` is a single self-contained page that loads every record parsed from
the SBEC Disciplinary Action Report PDFs published by the Texas Education
...
```
dar.html               Pre-built page served by GitHub Pages.
index.html             Redirects the site root to dar.html.
dar-reports/           Source PDFs from TEA / SBEC.
build/
  parse.py             Parses the PDFs into records.json.
  build_html.py        Composes dar.html from template.html + records.json.
  template.html        UI shell (styles + JS) for dar.html.
...
python3 build/build_html.py   # records.json + template.html -> dar.html
```
